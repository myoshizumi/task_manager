...
RSpec.describe 'タスク管理機能', type: :system do
  ...
  describe '一覧表示機能' do
    ...
    context 'Descending order by the priority' do
      ...
      let!(:task) do
        FactoryBot.create(:task, user: user_a, name: 'task', expired_at: '02/19', status: '完了', priority: '低')
      end
      let!(:task_second) do
        FactoryBot.create(:task, user: user_a, name: 'task_second', expired_at: '02/17', status: '未着手', priority: '中')
      end
      let!(:task_third) do
        FactoryBot.create(:task, user: user_a, name: 'task_third', expired_at: '02/15', status: '着手', priority: '高')
      end
